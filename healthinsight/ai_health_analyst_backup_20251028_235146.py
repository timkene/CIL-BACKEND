...
def render_dashboard():
    ...
    if not pa_data.empty and 'code' in pa_data.columns and not data['benefitcode_procedures'].empty and not data['benefitcodes'].empty:
        # Step 1: Join PA DATA with BENEFITCODE_PROCEDURES using code = benefitlevelid
        # Step 2: Join the result with BENEFITCODES using procedurecode = benefitcodeid
        benefit_analysis = pa_data.merge(
            data['benefitcode_procedures'],
            left_on='code',
            right_on='benefitlevelid',
            how='left'
        ).merge(
            data['benefitcodes'],
            left_on='procedurecode',
            right_on='benefitcodeid',
            how='left'
        )
