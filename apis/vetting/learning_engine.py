...
class LearningVettingEngine:
    ...
    def __init__(self, db_path: str = DB_PATH):
        ...
        self.anthropic_client = Anthropic(api_key=ANTHROPIC_API_KEY, max_retries=5, timeout=120.0) if ANTHROPIC_API_KEY else None
    ...
    def get_age_range_bounds(self, age_type: str) -> Optional[Tuple[int, int]]:
        ...
        FROM "PROCEDURE_DIAGNOSIS"."AGE_RANGE"
        WHERE UPPER(TRIM(age_type)) = UPPER(TRIM(?))
        LIMIT 1
        """
        result = self.conn.execute(query, [age_type.strip()]).fetchone()
        if result:
    ...
    def validate_age_for_procedure(
        ...
        """
        query = """
        SELECT procedure_name, procedure_class, age_range
        FROM "PROCEDURE_DIAGNOSIS"."PROCEDURE_MASTER"
        WHERE LOWER(TRIM(procedure_code)) = LOWER(TRIM(?))
        LIMIT 1
        """
        result = self.conn.execute(query, [procedure_code.strip()]).fetchone()
        if not result:
            return {
                'is_valid': None,
        ...
        proc_name, proc_class, age_range = result[0], result[1], result[2] if len(result) > 2 else None
    ...
    def validate_age_for_diagnosis(
        ...
        """
        query = """
        SELECT diagnosis_name, diagnosis_class, age_range
        FROM "PROCEDURE_DIAGNOSIS"."DIAGNOSIS_MASTER"
        WHERE LOWER(TRIM(diagnosis_code)) = LOWER(TRIM(?))
        LIMIT 1
        """
        result = self.conn.execute(query, [diagnosis_code.strip()]).fetchone()
        if not result:
            return {
                'is_valid': None,
        ...
        diag_name = result[0]
        diag_class = result[1] if len(result) > 1 else None
        age_range = result[2] if len(result) > 2 else None
        if not age_range or str(age_range).strip().upper() == 'ALL':
            return {
                'is_valid': True,
    ...
    def validate_gender_for_procedure(
        ...
        """
        query = """
        SELECT procedure_name, procedure_class, gender_applicable
        FROM "PROCEDURE_DIAGNOSIS"."PROCEDURE_MASTER"
        WHERE LOWER(TRIM(procedure_code)) = LOWER(TRIM(?))
        LIMIT 1
        """
        ...
        proc_name, proc_class, gender_code = result[0], result[1], result[2] if len(result) > 2 else None
        ...
        try:
            gender_desc_query = """
            SELECT description
            FROM "PROCEDURE_DIAGNOSIS"."GENDER_TYPE"
            WHERE LOWER(TRIM(gender_code)) = LOWER(TRIM(?))
            LIMIT 1
            """
            gender_desc_result = self.conn.execute(gender_desc_query, [gender_code]).fetchone()
            gender_description = gender_desc_result[0] if gender_desc_result else gender_code
        except:
            gender_description = gender_code
    ...
    def validate_gender_for_diagnosis(
        ...
        """
        query = """
        SELECT diagnosis_name, diagnosis_class, gender_applicable
        FROM "PROCEDURE_DIAGNOSIS"."DIAGNOSIS_MASTER"
        WHERE LOWER(TRIM(diagnosis_code)) = LOWER(TRIM(?))
        LIMIT 1
        """
        ...
        diag_name = result[0]
        diag_class = result[1] if len(result) > 1 else None
        gender_code = result[2] if len(result) > 2 else None
    ...
    def check_procedure_diagnosis_learning(
        ...
        result = self.conn.execute(query, [procedure_code, diagnosis_code]).fetchone()
        
        if result:
            # Update usage tracking
            self.conn.execute("""
                UPDATE "PROCEDURE_DIAGNOSIS"."ai_human_procedure_diagnosis"
                SET usage_count = usage_count + 1,
                    last_used_date = CURRENT_TIMESTAMP
                WHERE procedure_code = ? AND diagnosis_code = ?
    ...
    def check_procedure_class_learning(
        ...
        WHERE UPPER(TRIM(procedure_code_1)) = ? 
          AND UPPER(TRIM(procedure_code_2)) = ?
        """
        
        try:
            result = self.conn.execute(query, [code_1, code_2]).fetchone()

            if result:
                # Update usage
                self.conn.execute("""
                    UPDATE "PROCEDURE_DIAGNOSIS"."ai_human_procedure_class"
                    SET usage_count = usage_count + 1,
                        last_used_date = CURRENT_TIMESTAMP
                    WHERE UPPER(TRIM(procedure_code_1)) = ?
                      AND UPPER(TRIM(procedure_code_2)) = ?
    ...
    def check_diagnosis_class_learning(
        ...
        if result:
            # Update usage
            self.conn.execute("""
                UPDATE "PROCEDURE_DIAGNOSIS"."ai_human_diagnosis_class"
                SET usage_count = usage_count + 1,
                    last_used_date = CURRENT_TIMESTAMP
                WHERE code_1 = ? AND code_2 = ?
            """, [code_1, code_2])
        ...
        try:
            result = self.conn.execute("""
                SELECT 
                    ...
                    usage_count,
                    last_used
                FROM "PROCEDURE_DIAGNOSIS"."ai_human_procedure_age"
                WHERE LOWER(TRIM(procedure_code)) = LOWER(TRIM(?))
                  AND ? BETWEEN min_age AND max_age
                ORDER BY last_used DESC
                LIMIT 1
            """, [procedure_code, enrollee_age]).fetchone()
            
            if result:
                # Update usage count
                self.conn.execute("""
                    UPDATE "PROCEDURE_DIAGNOSIS"."ai_human_procedure_age"
                    SET usage_count = usage_count + 1,
                        last_used = CURRENT_TIMESTAMP
                    WHERE LOWER(TRIM(procedure_code)) = LOWER(TRIM(?))
                      AND ? BETWEEN min_age AND max_age
                """, [procedure_code, enrollee_age])
        ...
        try:
            result = self.conn.execute("""
                SELECT 
                    ...
                    usage_count,
                    last_used
                FROM "PROCEDURE_DIAGNOSIS"."ai_human_procedure_gender"
                WHERE LOWER(TRIM(procedure_code)) = LOWER(TRIM(?))
                  AND LOWER(TRIM(gender)) = LOWER(TRIM(?))
                ORDER BY last_used DESC
                LIMIT 1
            """, [procedure_code, enrollee_gender]).fetchone()
            
            if result:
                # Update usage count
                self.conn.execute("""
                    UPDATE "PROCEDURE_DIAGNOSIS"."ai_human_procedure_gender"
                    SET usage_count = usage_count + 1,
                        last_used = CURRENT_TIMESTAMP
                    WHERE LOWER(TRIM(procedure_code)) = LOWER(TRIM(?))
                      AND LOWER(TRIM(gender)) = LOWER(TRIM(?))
                """, [procedure_code, enrollee_gender])
                
                self.stats['learning_hits'] += 1
                
                return {
                    'allowed_gender': result[0],
                    'is_valid': result[1],
                    'reason': result[2],
                    'confidence': result[3],
                    'ai_reasoning': result[4],
        ...
        try:
            result = self.conn.execute("""
                SELECT 
                    ...
                    usage_count,
                    last_used
                FROM "PROCEDURE_DIAGNOSIS"."ai_human_diagnosis_age"
                WHERE LOWER(TRIM(diagnosis_code)) = LOWER(TRIM(?))
                  AND ? BETWEEN min_age AND max_age
                ORDER BY last_used DESC
                LIMIT 1
            """, [diagnosis_code, enrollee_age]).fetchone()
            
            if result:
                # Update usage count
                self.conn.execute("""
                    UPDATE "PROCEDURE_DIAGNOSIS"."ai_human_diagnosis_age"
                    SET usage_count = usage_count + 1,
                        last_used = CURRENT_TIMESTAMP
                    WHERE LOWER(TRIM(diagnosis_code)) = LOWER(TRIM(?))
                      AND ? BETWEEN min_age AND max_age
                """, [diagnosis_code, enrollee_age])
        ...
        try:
            result = self.conn.execute("""
                SELECT 
                    ...
                    usage_count,
                    last_used
                FROM "PROCEDURE_DIAGNOSIS"."ai_human_diagnosis_gender"
                WHERE LOWER(TRIM(diagnosis_code)) = LOWER(TRIM(?))
                  AND LOWER(TRIM(gender)) = LOWER(TRIM(?))
                ORDER BY last_used DESC
                LIMIT 1
            """, [diagnosis_code, enrollee_gender]).fetchone()
            
            if result:
                # Update usage count
                self.conn.execute("""
                    UPDATE "PROCEDURE_DIAGNOSIS"."ai_human_diagnosis_gender"
                    SET usage_count = usage_count + 1,
                        last_used = CURRENT_TIMESTAMP
                    WHERE LOWER(TRIM(diagnosis_code)) = LOWER(TRIM(?))
                      AND LOWER(TRIM(gender)) = LOWER(TRIM(?))
                """, [diagnosis_code, enrollee_gender])
    ...
    def check_procedure_master(self, procedure_code: str) -> Optional[Dict]:
        ...
        try:
            query = """
            SELECT procedure_name, procedure_class, frequency_category, clinical_notes
            FROM "PROCEDURE_DIAGNOSIS"."PROCEDURE_MASTER"
            WHERE procedure_code = ?
            """
    ...
    def check_diagnosis_master(self, diagnosis_code: str) -> Optional[Dict]:
        ...
        try:
            query = """
            SELECT diagnosis_name, diagnosis_class, implied_symptoms
            FROM "PROCEDURE_DIAGNOSIS"."DIAGNOSIS_MASTER"
            ...
            if not result:
                return None
                self.stats['master_hits'] += 1
                implied_sx_str = result[2] if len(result) > 2 and result[2] else ""
                implied_sx_list = [sx.strip().upper() for sx in implied_sx_str.split(',') if sx.strip()]
                return {
                'source': 'master_table',
                'name': result[0],
                'class': result[1],
    ...
    UNIVERSAL_PROCEDURE_CODES = {'CONS021', 'CONS022'}
    ...
    def check_procedure_diagnosis_comp(self, procedure_code: str, diagnosis_code: str) -> Optional[Dict]:
        ...
        try:
            query = """
            SELECT procedure_code, procedure_name, diagnosis_code, diagnosis_name
            FROM "PROCEDURE_DIAGNOSIS"."PROCEDURE_DIAGNOSIS_COMP"
            WHERE UPPER(procedure_code) = UPPER(?)
              AND UPPER(diagnosis_code) = UPPER(?)
            LIMIT 1
            """
            result = self.conn.execute(query, [procedure_code, diagnosis_code]).fetchone()
            if result:
                ...
                return {
                    'source': 'comp_table',
                    'procedure_code': result[0],
                    'procedure_name': result[1],
                    'diagnosis_code': result[2],
                    'diagnosis_name': result[3],
                    'is_valid': True,
                    'confidence': 100
                }
            return None
    ...
    def is_universal_procedure(self, procedure_code: str) -> bool:
        ...
        CONS021 = General Practitioner Consultation
        CONS022 = Follow-up Consultation
        These are used for every diagnosis — they should never fail proc-diag match.
        """
        return procedure_code.strip().upper() in self.UNIVERSAL_PROCEDURE_CODES
...
"""
    ...
    def _call_claude_for_validation(self, prompt: str) -> Dict:
        """
        Send validation prompt to Claude Haiku and parse JSON response.
        Returns dict with suggested_action, confidence, reasoning, requires_human_review.
        
        v3.6: Uses Haiku (faster, cheaper, less congested for JSON yes/no decisions).
              SDK handles retries internally — no custom retry loop.
        """
        if not self.anthropic_client:
            return {
                'suggested_action': 'DENY',
    ...
    def store_approved_decision(
        ...
        try:
            ...
            (procedure_code, diagnosis_code, is_valid_match, match_reason, 
             ai_confidence, ai_reasoning, approved_by, approved_date, 
             usage_count, last_used_date)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?)
            ON CONFLICT (procedure_code, diagnosis_code) 
            DO UPDATE SET
                is_valid_match = EXCLUDED.is_valid_match,
            ...
            self.conn.execute(query, [
                procedure_code, diagnosis_code, is_valid, reason,
                confidence, ai_reasoning, approved_by, now, now
            ])
    ...
    def store_procedure_age_decision(
        ...
        try:
            now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            query = """
            INSERT INTO "PROCEDURE_DIAGNOSIS"."ai_human_procedure_age"
            (procedure_code, min_age, max_age, is_valid_for_age, reason,
             confidence, ai_reasoning, approved_by, approved_date,
             usage_count, last_used)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?)
            ON CONFLICT (procedure_code, min_age, max_age)
            DO UPDATE SET
                is_valid_for_age = EXCLUDED.is_valid_for_age,
            ...
            self.conn.execute(query, [
                procedure_code, min_age, max_age, is_valid, reason,
                confidence, ai_reasoning, approved_by, now, now
            ])
    ...
    def store_procedure_gender_decision(
        ...
        try:
            now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            query = """
            INSERT INTO "PROCEDURE_DIAGNOSIS"."ai_human_procedure_gender"
            (procedure_code, gender, is_valid_for_gender, reason,
             confidence, ai_reasoning, approved_by, approved_date,
             usage_count, last_used)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?)
            ON CONFLICT (procedure_code, gender)
            DO UPDATE SET
                is_valid_for_gender = EXCLUDED.is_valid_for_gender,
                reason = EXCLUDED.reason,
                confidence = EXCLUDED.confidence,
                ai_reasoning = EXCLUDED.ai_reasoning,
                approved_by = EXCLUDED.approved_by,
            ...
            self.conn.execute(query, [
                procedure_code, allowed_gender, is_valid, reason,
                confidence, ai_reasoning, approved_by, now, now
            ])
    ...
    def store_diagnosis_age_decision(
        ...
        try:
            now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            query = """
            INSERT INTO "PROCEDURE_DIAGNOSIS"."ai_human_diagnosis_age"
            (diagnosis_code, min_age, max_age, is_valid_for_age, reason,
             confidence, ai_reasoning, approved_by, approved_date,
             usage_count, last_used)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?)
            ON CONFLICT (diagnosis_code, min_age, max_age)
            DO UPDATE SET
                is_valid_for_age = EXCLUDED.is_valid_for_age,
                reason = EXCLUDED.reason,
                confidence = EXCLUDED.confidence,
            ...
            self.conn.execute(query, [
                diagnosis_code, min_age, max_age, is_valid, reason,
                confidence, ai_reasoning, approved_by, now, now
            ])
    ...
    def store_diagnosis_gender_decision(
        ...
        try:
            now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            query = """
            INSERT INTO "PROCEDURE_DIAGNOSIS"."ai_human_diagnosis_gender"
            (diagnosis_code, gender, is_valid_for_gender, reason,
             confidence, ai_reasoning, approved_by, approved_date,
             usage_count, last_used)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?)
            ON CONFLICT (diagnosis_code, gender)
            DO UPDATE SET
                is_valid_for_gender = EXCLUDED.is_valid_for_gender,
                reason = EXCLUDED.reason,
                confidence = EXCLUDED.confidence,
            ...
            self.conn.execute(query, [
                diagnosis_code, allowed_gender, is_valid, reason,
                confidence, ai_reasoning, approved_by, now, now
            ])
    ...
    def store_procedure_class_decision(
        ...
        try:
            now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            # Sort pair to match lookup ordering
            code_1, code_2 = sorted([procedure_code_1.upper().strip(), procedure_code_2.upper().strip()])
            query = """
            INSERT INTO "PROCEDURE_DIAGNOSIS"."ai_human_procedure_class"
            (procedure_code_1, procedure_code_2, shared_class, same_class,
             ai_confidence, ai_reasoning, approved_by, approved_date,
             usage_count, last_used_date)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?)
            ON CONFLICT (procedure_code_1, procedure_code_2)
            DO UPDATE SET
                shared_class = EXCLUDED.shared_class,
                same_class = EXCLUDED.same_class,
                ai_confidence = EXCLUDED.ai_confidence,
                ai_reasoning = EXCLUDED.ai_reasoning,
                approved_by = EXCLUDED.approved_by,
                approved_date = EXCLUDED.approved_date
            ...
            self.conn.execute(query, [
                code_1, code_2, shared_class, same_class,
                ai_confidence, ai_reasoning, approved_by, now, now
            ])
...
"""
    ...
    def get_learning_stats(self) -> Dict:
        ...
        # Procedure-Diagnosis stats
        query = """
        SELECT 
            COUNT(*) as total_rules,
            SUM(usage_count) as total_reuses,
            AVG(usage_count) as avg_reuses_per_rule,
            SUM(CASE WHEN is_valid_match THEN 1 ELSE 0 END) as valid_count,
            SUM(CASE WHEN NOT is_valid_match THEN 1 ELSE 0 END) as invalid_count
        FROM "PROCEDURE_DIAGNOSIS"."ai_human_procedure_diagnosis"
        """
        result = self.conn.execute(query).fetchone()
        stats['procedure_diagnosis'] = {
            'total_rules': result[0],
            'total_reuses': result[1] or 0,
            'avg_reuses_per_rule': round(result[2] or 0, 1),
            'valid_count': result[3],
