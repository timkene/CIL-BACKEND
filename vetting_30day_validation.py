...
class ThirtyDayValidationEngine:
    ...
    def _get_30_day_procedures(self, enrollee_id: str, encounter_date: str) -> List[Dict]:
        """
        Get all procedures for this enrollee in the last 30 days
        ...
        date_30_days_ago = (encounter_dt - timedelta(days=30)).strftime('%Y-%m-%d')
    ...
    def _get_procedure_from_master(self, code: str) -> Optional[Dict]:
        ...
        WHERE LOWER(TRIM(procedure_code)) = '{code.lower().strip()}'
        LIMIT 1
        """
        ...
        try:
            ...
            if len(result) > 0:
                return {
                    'code': result.iloc[0]['procedure_code'],
                    'description': result.iloc[0]['procedure_name'],
                    'therapeutic_class': result.iloc[0]['procedure_class']
                }
        except Exception as e:
...
{{
  ...
  "reasoning": "brief explanation"
    ...
    def validate_procedure_30_day(
        ...
        # Step 2: Get 30-day history
        history_procedures = self._get_30_day_procedures(enrollee_id, encounter_date)
        ...
        for item in history_items:
            ...
            if item.classification_source == "master":
                # History from master → compare against input's master class
                compare_class = input_therapeutic_class
            else:
                # History from AI/PubMed → compare against input's AI class
                # (both classified in same AI batch = same taxonomy)
                compare_class = ai_input_class or input_therapeutic_class
            
            if compare_class in NON_CLASSES:
                continue
            
            if item.therapeutic_class == compare_class:
